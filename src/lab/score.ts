...
export const ATTACK_RELEASE_BEAT = 13 / 12;
...
export function samplePose(beat: number): Pose {
  ...
  const frame =
    seconds < 1 ? Math.min(23, Math.floor(seconds * FRAME_RATE + 1e-7)) : 0;
  let section = 'Ready',
  ...
  else if (frame >= 20) [section, cue] = ['Return', 'Rotate back to ready'];
  return {
    beat: b,
    section,
    cue,
